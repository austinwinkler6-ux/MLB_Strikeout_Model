...
KNOWN_WRONG_SLUG_REDIRECTS = {
    ...
    'vks': 'vivo-keyd-stars-academy',         # confirmed August 2026 — "vks" (Vivo Keyd Stars' real, shared slug) returns ALL real games for BOTH their main roster AND their Academy roster mixed together. Redirects specifically to the real, dedicated Academy slug so Academy-context matchups get a real, cleaner starting point — combined with the same real round-8 tier-filtering (now generalized to recognize "academy" too), this closes both real halves of the same real contamination pattern.
